...
A target-specific Skill generated by sp-platform from one Plugin source Skill. It owns the Plugin's business workflow rather than a Capability Domain and may call exact Action IDs through the Action namespace using the execution rules rendered into its body. The Platform Domain Skill ends before this Skill activates.
...
A Plugin distribution component that carries an executable dynamic Action rather than static content. A Generated Business Skill uses its exact ID through rendered Action-namespace rules and resumes its own workflow afterward. A Plugin with Actions but no Skill reaches the Action API Domain Skill through an explicit Domain Transition.
