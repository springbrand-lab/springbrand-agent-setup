...
Start a fresh session and verify that a covered request routes to `springbrand-resource-discovery` before planning or production work. The Hook is local, static, deterministic, prompt-stateless, and network-free; it only injects the routing instruction. Discovery itself happens through the Canonical Skill and MCP.
