# SpringBrand Dev OpenCode single-client installation
...
This document targets immutable development release `v1.2.1-dev.2`. It configures **OpenCode only**. Do not detect, configure, update, or remove any other client. Preserve every unrelated configuration and existing MCP entry.
...
```sh
npx -y @springbrand/cli@alpha connect opencode --url https://devconnector.springbrand.ai/mcp --api-key {{INSTALL_KEY}}
```
...
After the command completes, report the actual status without revealing credentials. Verify all of the following for **OpenCode**:
