...
three MCP Domain Entries per environment. The dev variant uses three
`springbrand-dev-*` entries against `devconnector.springbrand.ai`.
...
```text
Skill URLs:
  https://raw.githubusercontent.com/springbrand-lab/springbrand-agent-setup/v1.2.0-beta.7-dev.1/skills/ask-springbrand/SKILL.md
  https://raw.githubusercontent.com/springbrand-lab/springbrand-agent-setup/v1.2.0-beta.7-dev.1/skills/springbrand-platform/SKILL.md
  https://raw.githubusercontent.com/springbrand-lab/springbrand-agent-setup/v1.2.0-beta.7-dev.1/skills/springbrand-action-api/SKILL.md
  https://raw.githubusercontent.com/springbrand-lab/springbrand-agent-setup/v1.2.0-beta.7-dev.1/skills/springbrand-connector/SKILL.md
MCP URLs:
  https://devconnector.springbrand.ai/mcp/platform
  https://devconnector.springbrand.ai/mcp/action-api
  https://devconnector.springbrand.ai/mcp/connectors
```
...
| Version | `1.2.0-beta.7-dev.1` |
| Git ref | `v1.2.0-beta.7-dev.1` |
| MCP entries | `springbrand-dev-platform`, `springbrand-dev-action-api`, `springbrand-dev-connector` |
| MCP URLs | `https://devconnector.springbrand.ai/mcp/{platform,action-api,connectors}` |
| Transport | Native remote HTTP / Streamable HTTP |
| Authentication | Host-native OAuth (one consent per entry, up to three) |
...
not edit Plugin caches, override the bundled URLs, add static credentials, or
...
2. Use the Host-native Plugin lifecycle when available. The native package must
   ...
   three bundled `springbrand-dev-*` MCP entries.
3. Use manual Skill-plus-MCP installation only when the Host has no native
   ...
   paths.
4. Use Host-native OAuth. Each of the three entries requires its own consent —
   up to three per Surface. Never collect, print, proxy, or store tokens,
   authorization codes, client secrets, or other credentials.
5. Preserve unrelated Plugins, Skills, Hooks, Rules, MCP entries, OAuth state,
   ...
   entry; never alter the production `springbrand-*` entries from this guide.
...
```sh
codex plugin marketplace add springbrand-lab/springbrand-agent-setup --ref v1.2.0-beta.7-dev.1
codex plugin add springbrand-dev@springbrand-dev
codex mcp login springbrand-dev-platform
codex mcp login springbrand-dev-action-api
codex mcp login springbrand-dev-connector
```
...
```sh
claude plugin marketplace add springbrand-lab/springbrand-agent-setup@v1.2.0-beta.7-dev.1 --scope user
claude plugin install springbrand-dev@springbrand-dev --scope user
claude mcp login plugin:springbrand-dev:springbrand-dev-platform
claude mcp login plugin:springbrand-dev:springbrand-dev-action-api
claude mcp login plugin:springbrand-dev:springbrand-dev-connector
```

Desktop Code: open **Plugin Browser → Add Marketplace** and enter:

```text
springbrand-lab/springbrand-agent-setup@v1.2.0-beta.7-dev.1
```

Install **SpringBrand Dev**, complete OAuth for each of the three entries, and open a new Code task. This does not apply to Claude Chat, Cowork, web sessions, or account-level Connectors.
...
Install **SpringBrand Dev**, complete OAuth for each `springbrand-dev-*` entry, and open a new task.
...
browser step when WorkBuddy prompts the user for each `springbrand-dev-*` entry.

### Native Plugin verification and removal

Verify that the installed Plugin shows:

- Plugin ID `springbrand-dev` and version `1.2.0-beta.7-dev.1`;
- exactly three bundled `springbrand-dev-*` MCP entries;
- URLs exactly `https://devconnector.springbrand.ai/mcp/platform`,
  `https://devconnector.springbrand.ai/mcp/action-api`, and
  `https://devconnector.springbrand.ai/mcp/connectors`;
...
- `list_capabilities` on the `springbrand-dev-platform` entry resolves the
...
- `match_capabilities` on the `springbrand-dev-action-api` entry returns
  API Service candidates only;
- `search_capabilities` on the `springbrand-dev-connector` entry returns
...
Each of the three MCP Domain Entries requires its own OAuth consent — up to
three per Surface. Disclose this to the user before starting.
...
Use this path only when the host cannot install the native Plugin. Unlike two complete Plugins, this fallback may coexist with production because it shares the same Skill files and adds only the separately named `springbrand-dev-*` MCP entries.
...
1. **The four SpringBrand Skills** — `SKILL.md` files that teach the Agent the
   ...
   second copies.
2. **The three SpringBrand dev MCP servers** — remote MCP servers named
   `springbrand-dev-platform`, `springbrand-dev-action-api`, and
   `springbrand-dev-connector` at fixed dev URLs, which expose SpringBrand
   capabilities and connected providers in the development environment.

Each dev MCP server requires OAuth authorization before it can be used normally.
...
- **Skill sources:**
  ...
  - `https://raw.githubusercontent.com/springbrand-lab/springbrand-agent-setup/v1.2.0-beta.7-dev.1/skills/springbrand-action-api/SKILL.md`
  - `https://raw.githubusercontent.com/springbrand-lab/springbrand-agent-setup/v1.2.0-beta.7-dev.1/skills/springbrand-connector/SKILL.md`
- **MCP server names:** `springbrand-dev-platform`, `springbrand-dev-action-api`, `springbrand-dev-connector`
- **MCP server URLs (fixed):**
  - `https://devconnector.springbrand.ai/mcp/platform`
  - `https://devconnector.springbrand.ai/mcp/action-api`
  - `https://devconnector.springbrand.ai/mcp/connectors`
- **MCP transport:** native Streamable HTTP (remote HTTP transport, **not** a stdio bridge, **not** a local command)

If you were given different URLs, use the ones above. The dev URLs are fixed and must not be changed.
...
### Fallback Step 3 — Configure the SpringBrand dev MCP servers
...
3. For each of the three dev entries, look for an existing MCP server entry with that name:
...
4. **Preserve every other MCP server entry and all unrelated configuration.** Only the `springbrand-dev-*` entries may be added, replaced, or left alone. In particular, do **not** delete or modify the production `springbrand-*` entries if they exist — the two environments are meant to coexist, and the production entries are managed exclusively by `INSTALL.md`.

If your Agent only supports adding MCP servers through a UI and cannot edit its config file directly, tell the user the exact values to enter in the UI (the three names, URLs, and native HTTP transport from "Inputs" above) and **stop** after giving those instructions. Do not attempt to drive the UI yourself unless you have a verified ability to do so for that Agent.

If your Agent does not support remote Streamable HTTP MCP servers with OAuth, report that limitation and **stop**.

**Completion criterion:** the three `springbrand-dev-*` MCP servers are configured with their exact URLs using native HTTP transport, all other configuration (including any production `springbrand-*` entries) is intact, or you have stopped and reported an unsupported limitation.
...
2. **MCP config:** re-read and parse the configuration. Confirm each `springbrand-dev-*` entry has its exact URL from "Inputs" above and uses native Streamable HTTP transport (not a stdio bridge, not a local command).
3. Confirm no duplicate `springbrand-dev-*` entry exists.
4. Confirm no other MCP server entries were removed or altered — including the production `springbrand-*` entries, which must remain untouched.
...
3. The MCP configuration location you used and whether each `springbrand-dev-*` entry was added, replaced, or unchanged.
4. The full list of files you created or modified.
5. That they must **restart the Agent or open a new session** before the SpringBrand dev MCP servers are available — the current session will not rediscover the newly installed configuration.
6. That each `springbrand-dev-*` MCP server requires OAuth before it can be used normally. The user should complete authorization when the Agent prompts for it; the Agent decides when to trigger that flow.
...
- **Do not** change the MCP URLs. They are fixed at the `devconnector.springbrand.ai/mcp/{platform,action-api,connectors}` URLs for the dev environment.
- **Do not** name the MCP entries `springbrand-platform` etc. The dev entries must be named `springbrand-dev-platform` etc. so they can coexist with the production entries.
- **Do not** remove or alter any existing MCP server entry other than the `springbrand-dev-*` entries. In particular, do **not** delete or modify the production `springbrand-*` entries.
